<template>
    <div class=" background m-auto w-">
        <div class="flex justify-between w-80 mx-auto flex-col ">
            <span class="my-5 text-xl font-semibold">Listo para pelear</span>
            <button class="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700" @click="aPelear()">FIGHT!</button>
            <router-link to="fame" v-if="pokemonStore.gop == '2'" class="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Hall Of Fame</router-link>
        </div>
        <div class="flex mt-3">
            <div class="flex-none w-1/2 justify-center">
                <div v-if="pokemonStore.gop == '1'" :class="[pokemonStore.gop ? 'verde' : 'rojo']">Tu oponente Gano</div>
                <div v-if="pokemonStore.gop == '2'" :class="[pokemonStore.gop ? 'rojo' : 'verde']">Tu oponente Perdio</div>
                <div v-if="pokemonStore.gop == '3'">Empate</div>

                <div v-for="pokemon, index in pokemonStore.pokemons" :key="pokemon.id" >
                    <div class="border-2 border-purple-600 rounded-xl w-40 mx-auto my-2 ">
                        <span class="">{{pokemon.name}} -</span>
                        <span class=""> HP: {{pokemon.hp}}</span>
                    </div>

                    <div v-if="pokemonStore.resultadoss[index] == '1'" :class="[pokemonStore.resultadoss[index] ? 'verde' : 'rojo', 'default']">Ganador</div>
                    <div v-if="pokemonStore.resultadoss[index] == '2'" :class="[pokemonStore.resultadoss[index] ? 'rojo' : 'verde']">Perdedor</div>
                    <div v-if="pokemonStore.resultadoss[index] == '3'">Empate</div>
                    <img :src="pokemon.image" :alt="pokemon.name" class="m-auto" :class="[pokemonStore.resultados[index] == '2' ? 'pierde' : 'gana']">
    ...
    <div class="firefly"></div>
            <div class="flex-none w-1/2 justify-center">
                <div v-if="pokemonStore.gop == '2'" :class="[pokemonStore.gop ? 'verde' : 'rojo']">Ganaste</div>
                <div v-if="pokemonStore.gop == '1'" :class="[pokemonStore.gop ? 'rojo' : 'verde']">Perdiste</div>
                <div v-if="pokemonStore.gop == '3'">Empate</div>

                <div v-for="pokemon, index in pokemonStore.pokemonsPlayerUno" :key="pokemon.id">
                    <div class="border-2 border-purple-600 rounded-xl w-40 mx-auto my-2">
                        <span class="">{{pokemon.name}} -</span>
                        <span class=""> HP: {{pokemon.hp}}</span>
                    </div>

                    <div v-if="pokemonStore.resultados[index] == '2'" :class="[pokemonStore.resultados[index] ? 'verde' : 'rojo']">Ganador</div>
                    <div v-if="pokemonStore.resultados[index] == '1'" :class="[pokemonStore.resultados[index] ? 'rojo' : 'verde']">Perdedor</div>
                    <div v-if="pokemonStore.resultados[index] == '3'">Empate</div>
                    <img :src="pokemon.image" :alt="pokemon.name" class="m-auto" :class="[pokemonStore.resultados[index] == '1' ? 'pierde' : 'gana']">
...
<script setup>
import {usePokemonStore} from '../store/storePokedex';

const pokemonStore = usePokemonStore();

function aPelear(){
    pokemonStore.combate
    pokemonStore.ganadorOPerdedor 
}
console.log(pokemonStore.resultados)
pokemonStore.pokemonsRival()
pokemonStore.pokemonesAleatorio
...
<style scoped>
.background{
    background: linear-gradient(90deg, #ffffd5, #e4f4ff);
...
/* .gana{
    animation: gana 0.5s ease;
} */

.pierde{
    animation: pierdee infinite 1s ease;
}
.default{
...
@keyframes pierdee{
  ...
  100% {background: rgb(255, 108, 108)}
  100% {border-radius: 100px}
}

/* .toast-enter-active{ */
  /* transition:all 0.3s ease; */
  /* animation: wobble 20s ease; */
/* } */
/* .toast-leave-from{
  opacity:0;
